...
# 	@echo "Building manifest-tool"
# 	@GOPATH=$(abspath $(GOTOOLS_GOPATH)) GO111MODULE=on go get -d -u github.com/estesp/manifest-tool@v0.9.0
# 	@git -C $(abspath $(GOTOOLS_GOPATH))/src/github.com/estesp/manifest-tool checkout -q v0.9.0
# 	@GOPATH=$(abspath $(GOTOOLS_GOPATH)) GOBIN=$(abspath $(GOTOOLS_BINDIR)) go install -ldflags="-X main.version=0.9.0 -X main.buildDate=$$(date '+%Y-%m-%d')" github.com/estesp/manifest-tool/cmd/manifest-tool
# 	@git -C $(abspath $(GOTOOLS_GOPATH))/src/github.com/estesp/manifest-tool checkout -q master
...
# Lock to a versioned mockery
gotool.mockery: MOCKERY_VERSION ?= "v1.1.2"
gotool.mockery:
	@GOPATH=$(abspath $(GOTOOLS_GOPATH)) go get -d -u github.com/vektra/mockery
	@git -C $(abspath $(GOTOOLS_GOPATH))/src/github.com/vektra/mockery checkout -q $(MOCKERY_VERSION)
	@echo "Building github.com/vektra/mockery $(MOCKERY_VERSION) -> mockery"
	@GOPATH=$(abspath $(GOTOOLS_GOPATH)) GOBIN=$(abspath $(GOTOOLS_BINDIR)) go install -ldflags="-X main.version=$(MOCKERY_VERSION) -X main.buildDate=$$(date '+%Y-%m-%d')" github.com/vektra/mockery/cmd/mockery
	@git -C $(abspath $(GOTOOLS_GOPATH))/src/github.com/vektra/mockery checkout -q master
