...
func (v *TxValidator) Validate(block *cached.Block) error {
	...
	dataLength := len(block.Data.Data)
	...
	txsfltr := ledgerUtil.NewTxValidationFlags(dataLength)
	...
	// array of txids
	txidArray := make([]string, dataLength)

	results := make(chan *blockValidationResult, dataLength)
	go func() {
	...
	logger.Debugf("expecting %d block validation responses", dataLength)
