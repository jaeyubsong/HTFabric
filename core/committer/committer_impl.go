...
type PeerLedgerSupport interface {
	...
	CommitWithPvtData(blockAndPvtdata *ledger.BlockAndPvtData, committedTxs chan<- *deptype.Transaction) error
...
func (lc *LedgerCommitter) CommitWithPvtData(blockAndPvtData *ledger.BlockAndPvtData, committedTxs chan<- *deptype.Transaction) error {
	if infolab.DebugMode && (infolab.HashPut || infolab.ValidateAndPrepare) {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(core/ledger/committer/committer_impl.go) CommitWithPvtData called from %s#%d\n", file, no)
		}
	}
