...
func (e *Endorser) SimulateProposal(txParams *ccprovider.TransactionParams, cid *pb.ChaincodeID) (ccprovider.ChaincodeDefinition, *pb.Response, []byte, *pb.ChaincodeEvent, error) {
	...
	if !FirstCall {
		FirstCall = true
		var logpath = build.Default.GOPATH + "/../endorser.log"
	...
	atomic.AddUint64(&simCount, 1)
	blockCount := float64(infolab.MaxBlockSize/4.0)
	if (simCount % 6 == 0) {
		blockCalc := float64(simCount)/blockCount + 12
		// fmt.Printf("blockCount: %v\n", blockCount)
		// fmt.Printf("Cur simCount: %d, block: %.3f, diff: %.3f\n", simCount, blockCalc, blockCalc - float64(simulating.MostRecentCatchupBlockId))
		infolab.Log.Printf("Cur simCount: %d, block: %.3f, diff: %.3f\n", simCount, blockCalc, blockCalc - float64(simulating.MostRecentCatchupBlockId))
		// fmt.Printf("Cur simCount: %d, block: %.3f\n", simCount, float64(simCount-6)/250 + 7)
	}

	// fmt.Printf("Short Txid: %v\n", shorttxid(txParams.TxID))
	// fmt.Printf("Txid: %v\n", txParams.TxID)
	...
	if txParams.TXSimulator != nil {
		...
		res.KeyMetadataList = infolab.Put(simResult.PubSimulationResults)
		if infolab.EarlyResimulationEnabled && (config.IsStorage || config.IsEndorser) {
			for _, v := range res.KeyMetadataList {
				var baby = &ccprovider.SimulationParameter{
					TxParams:        txParams,
					CID:             cid,
					KeyMetadataList: res.KeyMetadataList,
				}
				simulating.RememberSimulation(string(v.Name), baby)
			}
			// 완료된 JTODO (200917) 여기서 endorsement peer가 자신이 시뮬레이션한 트랜잭션 목록에 트랜잭션을 추가해야 한다.
		}
...
func (e *Endorser) ProcessProposal(ctx context.Context, signedProp *pb.SignedProposal) (*pb.ProposalResponse, error) {
	...
	for (config.IsStorage || config.IsEndorser) && infolab.DeferredSimulation && simulating.DeferredState == simulating.DeferState {
		fmt.Printf("[Deferred] O_VAL: %d, P_VAL: %d, VerGap: %d\n", simulating.O_VAL, simulating.P_VAL, infolab.DeferredVersion)
		infolab.Log.Printf("[Deferred] O_VAL: %d, P_VAL: %d, VerGap: %d\n", simulating.O_VAL, simulating.P_VAL, infolab.DeferredVersion)
		simulating.DeferredCond.Wait()
	}
	if FirstCall {
		fmt.Printf("[Execute] O_VAL: %d, P_VAL: %d, VerGap: %d\n", simulating.O_VAL, simulating.P_VAL, infolab.DeferredVersion)
		infolab.Log.Printf("[Execute] O_VAL: %d, P_VAL: %d, VerGap: %d\n", simulating.O_VAL, simulating.P_VAL, infolab.DeferredVersion)
	}
