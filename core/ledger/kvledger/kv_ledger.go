...
func (l *kvLedger) CommitWithPvtData(pvtdataAndBlock *ledger.BlockAndPvtData, committedTxs chan<- *deptype.Transaction) error {
	if infolab.DebugMode && (infolab.HashPut || infolab.ValidateAndPrepare) {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(core/ledger/kvledger/kv_ledger.go) CommitWithPvtData called from %s#%d\n", file, no)
		}
	}
	var err error
	...
	if infolab.DebugMode && infolab.CheckMetric {
		fmt.Printf("(core/ledger/kvledger/kv_ledger.go) CommitWithPvtData txStatInfo length is %d\n", len(txstatsInfo))
	}
	...
	if !(config.IsEndorser || config.IsStorage) && blockNo > 1 {
		...
		// 실험 2
		// a, err := proto.Marshal(&gossip.Payload{Data: block.Block})
		// if err != nil {
		// 	panic(err)
		// }
		// pl := &gossip.Payload{}
	...
	elapsedUnlockBlockStorage := time.Since(startCommitBlockStorage)
	...
	// CommitBlockStorage mutex 없애면 이게 오래 걸림
	...
	go l.updateBlockStats(blockNo,
		elapsedBlockProcessing,
		elapsedCommitBlockStorage,
		elapsedUnlockBlockStorage,
		elapsedCommitState,
		txstatsInfo,
	)
	return nil
...
func (l *kvLedger) updateBlockStats(
	...
	blockstorageUnlockTime time.Duration,
