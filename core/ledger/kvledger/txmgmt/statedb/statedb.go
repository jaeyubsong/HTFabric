...
func (batch *UpdateBatch) Get(ns string, key string) *VersionedValue {
	if infolab.DebugMode && infolab.StateGet {
		...
		if ok {
			fmt.Printf("(core/ledger/kvledger/txmgmt/statedb/statedb.go) Get called from %s#%d\n", file, no)
		}
	}
...
func (batch *UpdateBatch) Put(ns string, key string, value []byte, version *version.Height) {
	if infolab.DebugMode && infolab.StatePut {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(core/ledger/kvledger/txmgmt/statedb/statedb.go) Put called from %s#%d\n", file, no)
		}
	}
...
func (batch *UpdateBatch) Exists(ns string, key string) bool {
	...
	// if countCur % 100 == 0 {
	// 	fmt.Printf("Exist called %d\n", len(batch.updates[ns].m))
	// }
