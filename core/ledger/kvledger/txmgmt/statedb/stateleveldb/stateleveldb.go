...
func (vdb *versionedDB) GetState(namespace string, key string) (*statedb.VersionedValue, error) {
	if infolab.DebugMode && infolab.HashGet {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(kvledger/txmgmt/statedb/stateleveldb/stateleveldb.go) GetState called from %s#%d\n", file, no)
		}
	}
...
func (vdb *versionedDB) ApplyUpdates(batch *statedb.UpdateBatch, height *version.Height) error {
	if infolab.DebugMode && infolab.HashPut {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(core/ledger/kvledger/txmgmt/statedb/stateleveldb/stateleveldb.go) ApplyUpdates called from %s#%d\n", file, no)
		}
	}
	// if height.BlockNum >  10{
	// 	return nil
	// }
