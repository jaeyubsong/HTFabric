...
func (txmgr *LockBasedTxMgr) ValidateAndPrepare(blockAndPvtdata *ledger.BlockAndPvtData, doMVCCValidation bool, committedTxs chan<- *deptype.Transaction) (
	[]*txmgr.TxStatInfo, error,
) {
	if infolab.DebugMode && infolab.ValidateAndPrepare {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_txmgr.go) ValidateAndPrepare from %s#%d\n", file, no)
		}
	}
...
func (txmgr *LockBasedTxMgr) Commit() error {
	if infolab.DebugMode && infolab.HashPut {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(core/ledger/kvledger/txmgmt/txmgr/lockbasedtxmgr/lockbased_txmgr.go) Commit called from %s#%d\n", file, no)
		}
	}
	...
	simulating.RecentlyUpdatedVersionedDB = txmgr.db
	txmgr.commitRWLock.Unlock()
...
func (txmgr *LockBasedTxMgr) CommitLostBlock(blockAndPvtdata *ledger.BlockAndPvtData) error {
	...
	if _, err := txmgr.ValidateAndPrepare(blockAndPvtdata, false, make(chan *deptype.Transaction, len(blockAndPvtdata.Block.Data.Data))); err != nil {
		return err
	}
