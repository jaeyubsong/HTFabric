...
type Support struct {
	...
	committer.Committer
	AnalyzerTable map[uint64]dependency.Analyzer
	TransientStore
	Fetcher
}
...
func NewCoordinator(support Support, selfSignedData common.SignedData, metrics *metrics.PrivdataMetrics,
	...
	return &coordinator{Support: support, selfSignedData: selfSignedData,
		...
		pullRetryThreshold: config.PullRetryThreshold, pipelineSemaphore: semaphore.NewWeighted(int64(runtime.NumCPU()))}
}

func (c *coordinator) Close() {
	for _, v := range c.AnalyzerTable {
		v.Stop()
	}
	c.Committer.Close()
}
...
func (c *coordinator) StoreBlock(block *cached.Block, privateDataSets util.PvtDataCollections) chan error {
	if infolab.DebugMode && (infolab.HashPut || infolab.ValidateAndPrepare) {
		...
		if ok {
			fmt.Printf("(gossip/privdata/coordinator.go) StoreBlock called from %s#%d\n", file, no)
		}
	}
	...
	var analyzer dependency.Analyzer
	var unblockedTx <-chan *deptype.Transaction
	c.pipelineSemaphore.Acquire(context.Background(), 1)

	c.lock.Lock()
	analyzer = dependency.NewAnalyzer(block.Header.Number)
	c.AnalyzerTable[block.Header.Number] = analyzer
	c.lock.Unlock()
	go func() {
		var err error
		unblockedTx, err = analyzer.Analyze(block)
		if err != nil {
			panic(err)
		}
		errAnalyze <- err
	...
	// 제발.
	// if block.Header.Number > 100 && !(config.IsEndorser || config.IsStorage) {
	// 	err := <-errAnalyze
	// 	if err != nil {
	// 		errOut <- err
	// 		return errOut
	// 	}
	// 	for tx := range unblockedTx {
	// 		tx.Dependencies = nil
	// 	}
	// 	errOut <- nil
	// 	return errOut
	...
	func() {
		...
		committedTxs := make(chan *deptype.Transaction, len(blockAndPvtData.Block.Data.Data))
		go func() {
			defer c.pipelineSemaphore.Release(1)
			var ptsts time.Duration = 0
			var ptstc time.Duration = 0
			...
			for tx := range committedTxs {
				...
				dur := infolab.ResolvePendingTransactionStartTime(tx.TxID)
				if dur > 0 {
					ptsts += dur
					ptstc++
				}
			...
			if ptstc > 0 {
				c.reportAverageTotalLatency(ptsts / ptstc)
			}
			...
			analyzer.Stop()
			// c.lock.Lock()
			// c.AnalyzerTable[seq].Stop()
			// delete(c.AnalyzerTable, seq)
			// c.lock.Unlock()
		}()
...
func (c *coordinator) reportAverageTotalLatency(time time.Duration) {
	c.metrics.TotalLatency.With("channel", c.ChainID).Observe(time.Seconds())
}
