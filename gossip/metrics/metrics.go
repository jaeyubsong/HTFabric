...
type StateMetrics struct {
	...
	CaliperLatency    metrics.Histogram
...
func newStateMetrics(p metrics.Provider) *StateMetrics {
	return &StateMetrics{
		...
		CaliperLatency:    p.NewHistogram(CaliperLatencyOpts),
...
var (
	...
	CaliperLatencyOpts = metrics.HistogramOpts{
		Namespace:    "gossip",
		Subsystem:    "state",
		Name:         "caliper_latency",
		Help:         "Hyperledger Caliper에서 정의한 latency (초)",
		LabelNames:   []string{"channel"},
		StatsdFormat: "%{#fqname}.%{channel}",
	}
...
type PrivdataMetrics struct {
	...
	TotalLatency                   metrics.Histogram
...
func newPrivdataMetrics(p metrics.Provider) *PrivdataMetrics {
	return &PrivdataMetrics{
		...
		TotalLatency:                   p.NewHistogram(TotalLatencyOpts),
...
var (
	...
	TotalLatencyOpts = metrics.HistogramOpts{
		Namespace:    "gossip",
		Subsystem:    "privdata",
		Name:         "total_latency",
		Help:         "클라이언트에서 proposal을 보낸 때부터 커밋이 완료되기까지의 시간 (초)",
		LabelNames:   []string{"channel"},
		StatsdFormat: "%{#fqname}.%{channel}",
	}
