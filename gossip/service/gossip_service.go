...
func InitGossipServiceCustomDeliveryFactory(peerIdentity []byte, metricsProvider metrics.Provider, endpoint string,
	...
	once.Do(func() {
		...
		simulating.GossipServiceInstance = gossipServiceInstance
	})
...
func (g *gossipServiceImpl) InitializeChannel(chainID string, endpoints []string, support Support) {
	...
	coordinator := privdata2.NewCoordinator(privdata2.Support{
		...
		TransientStore:  support.Store,
		Committer:       support.Committer,
		AnalyzerTable:   support.AnalyzerTable,
		Fetcher:         fetcher,
...
func (g *gossipServiceImpl) AddPayload(chainID string, payload *cached.GossipPayload) error {
	...
	if infolab.DebugMode && infolab.GossipPayload {
		...
		if ok {
			fmt.Printf("(gossip/service/gossip_service.go) AddPayload called from %s#%d\n", file, no)
		}
	}
	return g.chains[chainID].AddPayload(payload)
}

func (g *gossipServiceImpl) SendToFastPeer(channelID string, message *proto.GossipMessage) error {
	return g.chains[channelID].SendToFastPeer(message)
}
