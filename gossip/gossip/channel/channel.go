...
func (gc *gossipChannel) HandleMessage(msg proto.ReceivedMessage) {
	...
	// 완료된 JTODO (200917) endorsement peer가 보낸 다시 시뮬레이션된 트랜잭션들을 여기서 fast peer가 받아 해시 테이블 등에 기록해야 한다.
	if m.IsResimulationResultMsg() {
		result := m.GetResimulationResult()
		// fmt.Printf("조기 재시뮬레이션 결과 수신: %s\n", result.TransactionId)
		simulating.RememberResimulation(result)
		return
	}
	if m.IsCatchUpMsg() {
		data := m.GetCatchUpSignal()
		switch data.Type {
		case proto.CatchUpSignal_START:
			if !(config.IsEndorser || config.IsStorage) {
				go simulating.StartCatchUp()
			}
		case proto.CatchUpSignal_BODY:
			if config.IsEndorser {
				...
				if (data.BlockId < math.MaxUint64 && data.BlockId >= math.MaxUint64 - 10000000) {
					newBlockId := data.BlockId - (math.MaxUint64 - 10000000)
					simulating.ReceiveFastWb(newBlockId, data.Payload)
					fmt.Printf("Received fastWB with block %d\n", newBlockId)
					infolab.Log.Printf("[FastWB 수신]: %d\n", newBlockId)
				} else {
					simulating.ReceiveCatchUpBody(data.BlockId, data.Payload)
					fmt.Printf("Received catchupBody with block %d\n", data.BlockId)
				}
			}
		}
		return
		...
		if m.IsDataMsg() {
			if m.GetDataMsg().Payload == nil {
				if infolab.DebugMode {
					fmt.Printf("Datamsg payload is empty\n")
				}
			...
			if infolab.DebugMode && infolab.MsgRecvTime {
				...
				if receiveCount == 3300 {
					receivePeriodAverage = receivePeriodAverage / 3000
					fmt.Printf("**Datamsg receive average is %d\n", receivePeriodAverage)
				}
			}
			if infolab.DebugMode {
				fmt.Printf("Data message with blockNum: %d\n", m.GetDataMsg().Payload.Data.Header.Number)
			}
			if !(config.IsEndorser || config.IsStorage) {
				// 제발.
				// defer func() {
