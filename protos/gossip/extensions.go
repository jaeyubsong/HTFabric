...
// IsResimulationResultMsg 함수는 이 메시지가 조기 재시뮬레이션 결과를 담고 있는지 여부를 반환한다.
func (m *GossipMessage) IsResimulationResultMsg() bool {
	return m.GetResimulationResult() != nil
}

// IsCatchUpMsg 함수는 이 메시지가 캐치업 정보를 담고 있는지 여부를 반환한다.
func (m *GossipMessage) IsCatchUpMsg() bool {
	return m.GetCatchUpSignal() != nil
}
...
func (m *GossipMessage) IsTagLegal() error {
	...
	if m.IsStateInfoMsg() || m.IsStateInfoPullRequestMsg() || m.IsStateInfoSnapshot() || m.IsRemoteStateMessage() || m.IsResimulationResultMsg() || m.IsCatchUpMsg() {
