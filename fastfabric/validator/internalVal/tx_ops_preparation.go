...
func prepareTxOps(rwset *cached.TxRwSet, txht *version.Height,
	...
	if infolab.DebugMode && infolab.StateGet {
		...
		if ok {
			fmt.Printf("(fabric/fastfabric/validator/internalVal/tx_ops_preparation.go) prepareTxOps called from %s#%d\n", file, no)
		}
	}
	txops := txOps{}
...
func retrieveLatestMetadata(ns, coll, key string,
	...
	if infolab.DebugMode && infolab.StateGet {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(fabric/fastfabric/validator/internalVal/tx_ops_preparation.go) retrieveLatestMetadata called from %s#%d\n", file, no)
		}
	}
	if coll == "" {
		vv := precedingUpdates.PubUpdates.Get(ns, key)
