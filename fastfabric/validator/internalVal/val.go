...
type Validator interface {
	ValidateAndPrepareBatch(block *Block, doMVCCValidation bool, committedTxs chan<- *deptype.Transaction) (*PubAndHashUpdates, []uint64, []uint64, error)
}
...
func (u *PubAndHashUpdates) ApplyWriteSet(txRWSet *cached.TxRwSet, txHeight *version.Height, db privacyenabledstate.DB) error {
	if infolab.DebugMode && infolab.StateGet {
		_, file, no, ok := runtime.Caller(1)
		if ok {
			fmt.Printf("(fabric/fastfabric/validator/internalVal/val.go) ApplyWriteSet called from %s#%d\n", file, no)
		}
	}
