...
async function main(){
  ...
  var bcResps: Array<Promise<number>> = [];
  ...
  try {
    ...
    while(true){
      ...
      const data = requestHandler(chunk);
      ...
      if (requestCount > 0 && requestCount % NUM_TPS == 0) {
        ...
        send_tps_count++
        // await sleep_ms(getRandomArbitrary(1, 10 * NUM_TPS))
