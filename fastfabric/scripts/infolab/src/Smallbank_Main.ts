...
async function main(){
  ...
  const queryHandler = chunk => ({
    fcn: "query",
    args: [ chunk[0] ]
  });

  const sendOneHandler = chunk => ({
    fcn: "send1",
    args: [ chunk[0] ]
  });

  const TransactionSavingsHandler = chunk => ({
    fcn: "send1",
    args: [ (Number(chunk[0]) * 2).toString(10) ]
  });

  const DepositCheckingHandler = chunk => ({
    fcn: "send1",
    args: [ chunk[0] ]
  });

  const SendPaymentHandler = chunk => ({
    fcn: "send2",
    args: [ chunk[0], chunk[1], chunk[2] ]
  });

  const WriteCheckHandler = chunk => ({
    fcn: "send1",
    args: [ chunk[0] ]
  });

  const AmalgmateHandler = chunk => ({
    fcn: "send2",
    args: [ chunk[0], (Number(chunk[0]) * 2).toString(10), chunk[2] ]
  });
  ...
  try {
    ...
    while(true){
      ...
      if (requestCount % 20 >= Pw) {
        // console.log("This is query")
        data = queryHandler(chunk)
      } else {
        writeCount++
        let cur = 0
        // console.log(`This is write with ${cur}`)
        switch(cur) {
          case 0: // Transaction savings
            // console.log("Transaction saving")
            data = TransactionSavingsHandler(chunk);
            break;
          case 1: // Deposit Checking
            // console.log("Deposit Checking")
            data = DepositCheckingHandler(chunk);
            break;
          case 2: // SendPayment
            data = SendPaymentHandler(chunk);
            break;
          case 3: // WriteCheck
            data = WriteCheckHandler(chunk);
            break;
          case 4: // Amalgmate
            data = AmalgmateHandler(chunk);
            break;
          default:
            console.log("Error")
        }
