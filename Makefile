...
CHAINTOOL_URL ?= https://github.com/hyperledger/fabric-chaintool/releases/download/v0.10.3/chaintool
...
PROJECT_FILES = $(shell git ls-files  | grep -v ^test | grep -v ^unit-test | \
	...
	grep -v ^LICENSE | grep -v ^vendor | grep -v node_modules )
...
$(BUILD_DIR)/image/tools/$(DUMMY): $(BUILD_DIR)/image/tools/Dockerfile
	...
	docker tag $(DOCKER_NS)/fabric-$(TARGET) $(DOCKER_NS)/fabric-$(TARGET):latest
	# docker push $(DOCKER_NS)/fabric-$(TARGET):latest
	@touch $@
