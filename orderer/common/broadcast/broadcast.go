...
var first bool
var processMsgCount int64
var process_startTime time.Time
var process_lastTime time.Duration
var process_period time.Duration
var process_period_avg int64

// ProcessMessage validates and enqueues a single message
func (bh *Handler) ProcessMessage(msg *cb.Envelope, addr string) (resp *ab.BroadcastResponse) {
	if !first {
		process_startTime = time.Now()
		process_lastTime = time.Since(process_startTime)
	}
	// if infolab.DebugMode && infolab.Orderer_processMsgTime {
	// 	_, file, no, ok := runtime.Caller(1)
	// 	if ok {
	// 		fmt.Printf("(orderer/common/broadcast/broadcast.go) ProcessMessage() called from %s#%d\n", file, no)
	...
	if !isConfig {
		if infolab.DebugMode && infolab.Orderer_processMsgTime {
			processMsgCount++
			process_period = time.Since(process_startTime) - process_lastTime
			process_lastTime = time.Since(process_startTime)
			if processMsgCount == 1000000 {
				process_period_avg = process_period_avg / (1000000 - 10000)
				fmt.Printf("** processMsgCount average is %d\n", process_period_avg)
			} else if processMsgCount > 10000 {
				process_period_avg += process_period.Nanoseconds()
				if processMsgCount%10000 == 0 {
					fmt.Printf("processMsgCount [%d], %d left\n", processMsgCount, 1000000-processMsgCount)
				}
			}
