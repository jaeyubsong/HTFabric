...
type Collector struct {
	...
	HeapAllocE     metrics.Gauge
	HeapAllocS     metrics.Gauge
...
func NewCollector(p metrics.Provider) *Collector {
	return &Collector{
		...
		HeapAllocE:     p.NewGauge(heapAllocGaugeOptsE),
		HeapAllocS:     p.NewGauge(heapAllocGaugeOptsS),
...
func (c *Collector) Publish(stats Stats) {
	...
	if config.IsEndorser{
		c.HeapAllocE.Set(float64(stats.MemStats.Alloc))
	}else if config.IsStorage{
		c.HeapAllocS.Set(float64(stats.MemStats.Alloc))
	}else{
		c.HeapAlloc.Set(float64(stats.MemStats.Alloc))
	}
